            ...
            <div class="col-12">
              <div class="card">
                ...
                <form class="form form-horizontal"
                  action="{{ route('backsite.asset-indicator.update', $assetIndicator) }}" method="POST"
                  ...
                  <div class="form-body">
                        ...
                        <select id="type_asset_id" name="type_asset_id" class="form-control select2" required>
                          ...
                          @foreach ($typeAssets as $typeAsset)
                            <option value="{{ $typeAsset->id }}"
                              {{ old('type_asset_id', $assetIndicator->type_asset_id) == $typeAsset->id ? 'selected' : '' }}>
                              {{ $typeAsset->name }}
                            </option>
                          @endforeach
                        </select>
                    ...
                    <div class="form-group row col-10">
                      ...
                      <div class="col-md-9 mx-auto">
                        <input type="text" id="name" name="name" class="form-control" placeholder=""
                          value="{{ old('name', $assetIndicator->name) }}" autocomplete="off" autofocus required>
                      </div>
                      @if ($errors->has('name'))
                        <p style="font-style: bold; color: red;">
                          {{ $errors->first('name') }}</p>
                    ...
                    <div class="form-group row col-10">
                      ...
                      <div class="col-md-9 mx-auto">
                        <textarea class="form-control" id="description" name="description" rows="5">{{ old('description', $assetIndicator->description) }}</textarea>
                      </div>
                    ...
                    {{-- <div class="form-group row col-10">
                      ...
                      <div class="col-md-2">
                        <select name="status" id="status" class="form-control select2" required>
                          ...
                          <option value="1" {{ $assetIndicator->status == 1 ? 'selected' : '' }}>
                            Aktif
                          </option>
                          <option value="2" {{ $assetIndicator->status == 2 ? 'selected' : '' }}>
                            Tidak Aktif</option>
                        </select>
                      </div>
